...
do
   ...
   # echo "Deploying ${image_path} (git ${CI_COMMIT_TAG:-$CI_COMMIT_REF_NAME} $CI_COMMIT_SHA)"
    set -x
    helm install ${service_name} helm/spring-petclinic-kubernetes --set="${INGRESS_OVERRIDE}fullnameOverride=${service_name}"  --set "image.repository=${image_path},image.tag=latest" --namespace spc -f helm/spring-petclinic-kubernetes/values.${service_name}.yaml
